...
double** bspderiv(int p, double cp[][2], int num_cp, double U[], int num_U)
{
	...
	U - knot sequence
	num_U - number of the elements in knot sequence
...
// 6. a1_length
double a1_length(int p, double cp[][2], int num_cp, double U[], int num_U, double u[], int num_sp)
{
	/*
	a1_length - calculate the length of the B-spline
	...
	num_U - number of the elements in knot sequence
	u - parametric evaluation points.
	num_sp - number of sampling points

	Meaning of the output:
	length1 - length of the B-spline
	*/
	...
	for (int i = 0; i < 2; i++)
	{
		pnt[i] = (double*)malloc(num_sp * sizeof(double));
	}
	temp1 = bspeval(p, cp, num_cp, U, num_U, u, num_sp);
	for (i = 0; i < 2; i++)
	{
		for (j = 0; j < num_sp; j++)
		{
	...
	{
		length1 = length1 + sqrt(pow((pnt[0][i + 1] - pnt[0][i]), 2) + pow((pnt[1][i + 1] - pnt[1][i]), 2));
		// printf("length1[%d] is %f\n", i, length1);
	}
	return length1;
}
// end of a1_length
...
int main()
{
	...
	*/
    ...
    double length1;
    int p = 3;
    double cp[6][2] = { {0,   0}, {10,  15}, {20, -15}, {30,  15}, {40, -15}, {50,  0} };
    int num_cp = 6;
    double U[10] = { 0, 0, 0, 0, 0.333, 0.667, 1, 1, 1, 1 };
    int num_U = 10;
    int num_sp = 100;
    ...
    double* u;
    double* temp1;
    u = (double*)malloc(sizeof(double) * num_sp);
    temp1 = linspace(0, 1, num_sp);
    for (i = 0; i < num_sp; i++)
    {
	    u[i] = *(temp1 + i);
    }

	length1 = a1_length(p, cp, num_cp, U, num_U, u, num_sp);
	printf("length1 equals to %f", length1);
	*/
